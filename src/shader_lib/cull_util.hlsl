...
bool mehslet_bitfield_check_bit(
    MeshletInstance meshlet_instance,
    FirstPassMeshletBitfield* meshlet_bitfield
)
{
    if (meshlet_instance.mesh_instance_index >= FIRST_PASS_MESHLET_BITFIELD_U32_OFFSETS_SIZE) // Out of bounds
    {
        return false;
    }

    uint offset = meshlet_bitfield.meshlet_instance_offsets[meshlet_instance.mesh_instance_index];
    if (offset == 0)
    {
        return false;
    }
    ...
    uint u32_index = meshlet_instance.meshlet_index / 32;
    uint u32_bit_index = meshlet_instance.meshlet_index % 32;
    uint bitfield_u32 = meshlet_bitfield.bitfield[offset + u32_index];
    bool is_visibility_bit_set = ((bitfield_u32 >> u32_bit_index) & 1u) != 0;
    return is_visibility_bit_set;
}

void mehslet_bitfield_set_bit(
    MeshletInstance meshlet_instance,
    FirstPassMeshletBitfield* meshlet_bitfield
)
{
    ...
    uint bitfield_u32 = 1u << u32_bit_index;
    InterlockedAdd(meshlet_bitfield.bitfield[offset + u32_index], bitfield_u32);
}
