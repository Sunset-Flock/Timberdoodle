...
)
{
    ...
    {
        const vec2 f_hiz_resolution = vec2(deref(push.globals).settings.render_target_size >> 1 /*hiz is half res*/);
        ...
        const ivec2 mip_size = max(ivec2(0,0),ivec2(deref(push.globals).settings.render_target_size >> (1 + imip)) - 1);
