...
bool is_out_of_frustum(vec3 ws_center, float ws_radius)
{
    const vec3 frustum_planes[5] = {
        push.globals.camera_right_plane_normal,
        push.globals.camera_left_plane_normal,
        push.globals.camera_top_plane_normal,
        push.globals.camera_bottom_plane_normal,
        push.globals.camera_near_plane_normal,
    };
    bool out_of_frustum = false;
    for (uint i = 0; i < 5; ++i)
    {
        out_of_frustum = out_of_frustum || (dot((ws_center - push.globals.camera_pos), frustum_planes[i]) - ws_radius) > 0.0f;
    }
...
bool is_tri_out_of_frustum(vec3 tri[3])
{
    ...
    for (uint i = 0; i < 5; ++i)
    {
        ...
        {
            tri_out_of_plane = tri_out_of_plane && dot((tri[ti] - push.globals.camera_pos), frustum_planes[i]) > 0.0f;
        }
...
)
{
    ...
    const vec3 center_to_camera = normalize(push.globals.camera_pos - ws_center);
    const vec3 tangential_up = normalize(push.globals.camera_up - center_to_camera * dot(center_to_camera, push.globals.camera_up));
    ...
    for (int z = -1; z <= 1; z += 2)
    {
        for (int y = -1; y <= 1; y += 2)
        {
            for (int x = -1; x <= 1; x += 2)
            {
                ...
                const vec4 projected_pos = push.globals.camera_view_projection * model_matrix * vec4(bounding_box_corner_ws, 1);
    ...
    {
        const vec2 f_hiz_resolution = vec2(push.globals.settings.render_target_size >> 1 /*hiz is half res*/);
        ...
        const ivec2 mip_size = max(ivec2(0,0),ivec2(push.globals.settings.render_target_size >> (1 + imip)) - 1);
