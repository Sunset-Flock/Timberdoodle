    ...
    if(all(lessThan(svdtid.xy, int2(VSM_META_MEMORY_TABLE_RESOLUTION))))
    {
        ...
        if (get_meta_memory_is_allocated(meta_entry))
        {
            if(get_meta_memory_is_point_spot_light(meta_entry))
            {
                ...
                {
                    push.vsm_point_spot_page_table[coords.mip_level].get()[int3(coords.texel_coords, coords.array_layer_index)] = 0;
                    push.vsm_meta_memory_table.get()[svdtid.xy] = 0;
                }
                let entry = push.vsm_point_spot_page_table[coords.mip_level].get()[int3(coords.texel_coords, coords.array_layer_index)];
                let reset_entry = entry & (~dirty_mask());
                push.vsm_point_spot_page_table[coords.mip_level].get()[int3(coords.texel_coords, coords.array_layer_index)] = reset_entry;
        ...
        if(get_meta_memory_is_visited(meta_entry))
        {
            ...
            if(get_meta_memory_is_point_spot_light(meta_entry))
            {
                ...
                const uint virtual_entry = push.vsm_point_spot_page_table[coords.mip_level].get()[int3(coords.texel_coords, coords.array_layer_index)];

                const uint reset_virtual_entry = virtual_entry & (~(visited_marked_mask() | dirty_mask() | requests_allocation_mask()));
                push.vsm_point_spot_page_table[coords.mip_level].get()[int3(coords.texel_coords, coords.array_layer_index)] = reset_virtual_entry;
            }
            else
            {
                const int3 virtual_page_table_coords = get_vsm_coords_from_meta_entry(meta_entry);
                ...
                const uint virtual_entry = push.vsm_page_table.get()[virtual_page_table_coords];
                const uint reset_virtual_entry = virtual_entry & (~(visited_marked_mask() | dirty_mask() | requests_allocation_mask()));
                push.vsm_page_table.get()[virtual_page_table_coords] = reset_virtual_entry;
            }
        }
