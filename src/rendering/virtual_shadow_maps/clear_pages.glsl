...
DAXA_DECL_PUSH_CONSTANT(ClearPagesH, push)
...
void main()
{
    ...
    {
        const ivec3 alloc_request_page_coords = deref_i(push.vsm_allocation_requests, gl_GlobalInvocationID.z).coords;
        vsm_page_entry = imageLoad(daxa_uimage2DArray(push.vsm_page_table), alloc_request_page_coords).r;
        const uint vsm_page_entry_marked_dirty = vsm_page_entry | dirty_mask();
        imageStore(daxa_uimage2DArray(push.vsm_page_table), alloc_request_page_coords, daxa_u32vec4(vsm_page_entry_marked_dirty));
    }
    ...
    imageStore(daxa_image2D(push.vsm_memory), thread_memory_coords, vec4(1.0));
}