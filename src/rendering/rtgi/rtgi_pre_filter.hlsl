...
[[vk::push_constant]] RtgiPreFilterApplyPush rtgi_reconstruct_history_apply_diffuse_push;
...
groupshared float gs_firefly_power[RTGI_PRE_BLUR_PREPARE_X][RTGI_PRE_BLUR_PREPARE_Y];
...
func downsample_mip_linear(uint2 thread_index, uint2 group_thread_index, uint mip, uint gs_src)
{
    ...
    if (all(group_thread_index.xy < remaining_block_size))
    {
        ...
        const float4 brightness_weights = {
            float(mip == 1) * gs_firefly_power[group_thread_index.x * 2 + 0][group_thread_index.y * 2 + 0],
            float(mip == 1) * gs_firefly_power[group_thread_index.x * 2 + 1][group_thread_index.y * 2 + 0],
            float(mip == 1) * gs_firefly_power[group_thread_index.x * 2 + 0][group_thread_index.y * 2 + 1],
            float(mip == 1) * gs_firefly_power[group_thread_index.x * 2 + 1][group_thread_index.y * 2 + 1]
            // sqrt(gs_diffuse[group_thread_index.x * 2 + 0][group_thread_index.y * 2 + 0][gs_src].w),
            // sqrt(gs_diffuse[group_thread_index.x * 2 + 1][group_thread_index.y * 2 + 0][gs_src].w),
            // sqrt(gs_diffuse[group_thread_index.x * 2 + 0][group_thread_index.y * 2 + 1][gs_src].w),
            // sqrt(gs_diffuse[group_thread_index.x * 2 + 1][group_thread_index.y * 2 + 1][gs_src].w)
        };
        ...
        float4 weights = {
            (depths[0] != 0.0f ? 1.0f : 0.0f) * ((depths[0] == max_depth && ((depths[0] - min_depth) > 0.001f)) ? 0 : 1) * brightness_weights[0],
            (depths[1] != 0.0f ? 1.0f : 0.0f) * ((depths[1] == max_depth && ((depths[1] - min_depth) > 0.001f)) ? 0 : 1) * brightness_weights[1],
            (depths[2] != 0.0f ? 1.0f : 0.0f) * ((depths[2] == max_depth && ((depths[2] - min_depth) > 0.001f)) ? 0 : 1) * brightness_weights[2],
            (depths[3] != 0.0f ? 1.0f : 0.0f) * ((depths[3] == max_depth && ((depths[3] - min_depth) > 0.001f)) ? 0 : 1) * brightness_weights[3],
        };
...
func entry_prepare(uint2 dtid : SV_DispatchThreadID, uint2 gtid : SV_GroupThreadID)
{
    ...
    for (int x = -FILTER_WIDTH; x <= FILTER_WIDTH; ++x)
    {
        for (int y = -FILTER_WIDTH; y <= FILTER_WIDTH; ++y)
        {
            ...
            if (!is_sky && (sample_diffuse.w != 0.0f) && !(x == 0 && y == 0))
            {
    ...
    if (valid_neightborhood_samples > 1.0f)
    {
        ...
        y_mean_acc += filtered_diffuse.w;
        y_variance_acc += square(filtered_diffuse.w);
        y_max = max(y_max, filtered_diffuse.w);
        const float y_mean = y_mean_acc * rcp(valid_neightborhood_samples + 1);
        ...
        push.attach.debug_image.get()[dtid] = float4(firefly_energy_factor, foreground_footprint_quality, 0, 0);
        // push.attach.debug_image.get()[dtid * 2 + uint2(0,0)] = float4(y_std_dev_relative, foreground_footprint_quality, 0, 0);
        // push.attach.debug_image.get()[dtid * 2 + uint2(1,1)] = float4(y_std_dev_relative, foreground_footprint_quality, 0, 0);
        // push.attach.debug_image.get()[dtid * 2 + uint2(1,0)] = float4(y_std_dev_relative, foreground_footprint_quality, 0, 0);
    }
    ...
    gs_firefly_power[gtid.x][gtid.y] = firefly_energy_factor;
...
func entry_apply(uint2 dtid : SV_DispatchThreadID, uint2 gtid : SV_GroupThreadID)
{
    ...
    float plane_pixel_dist_acceptance_threshold = 1.0f;
    {
        ...
        const float disocclusion_fix_value = 1;//min(1.0f, pixel_samplecnt * (1.0f / disocclusion_blur_frames));
