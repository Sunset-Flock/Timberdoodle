...
func entry_post_blur(uint2 dtid : SV_DispatchThreadID)
{
    ...
    const float raylength_guide = rtgi_settings.post_blur_raylength_guiding ? lerp(rtgi_settings.raylength_guide_floor, 1.0f, filter_guide) : 1.0f;
    ...
    const int filter_width = max(1, int((float)rtgi_settings.post_blur_max_width * max(frame_scale, raylength_guide)));
    ...
    const float sample_count_ramp = rtgi_settings.history_frames * 0.1f;
    const float geo_mean_guide_ramp = saturate((pixel_samplecnt - sample_count_ramp) / max(sample_count_ramp, 1.0f));
    ...
    for (int i = -filter_width; i <= filter_width; i += rtgi_settings.post_blur_stride)
    {
        ...
        const float temporal_geo_factor = rtgi_settings.post_blur_geometric_luma_guiding_factor * geo_mean_guide_ramp;
...
#endif
        const float temporal_geo_weight = (rtgi_settings.post_blur_geometric_luma_guiding && !is_near_center) ? exp(-square(temporal_geo_factor * 2.0f * temporal_geo_log_ratio)) : 1.0f;
        ...
        const float sample_count_ramp = rtgi_settings.history_frames * 0.15f;
        const float variance_guide_ramp = saturate((pixel_samplecnt - sample_count_ramp) / max(sample_count_ramp, 1.0f));
        ...
        const float weight = geometric_weight * gauss_weight * temporal_geo_weight;// * one_sided_luminance_weight;
...
func entry_atrous_post_blur(uint2 dtid : SV_DispatchThreadID)
{
    ...
    const float filter_guide    = push.attach.filter_guide_image.get()[dtid];

    // Raylength guide is calibrated for the pre blur that has a radius of 64, we take the lowest 25% of the guide as that rouughly matches our range here of 8-16 pixels.
    const float raylength_guide = min(0.25f, rtgi_settings.post_blur_raylength_guiding ? lerp(rtgi_settings.raylength_guide_floor, 1.0f, filter_guide) : 1.0f) * 4.0f;
    // Temporal stability pushes toward full blur when sample count is low (disocclusion)
    const float effective_raylength = lerp(raylength_guide, 1.0f, temporal_stability_scale);
    ...
    for (int dy = -1; dy <= 1; ++dy)
    {
        for (int dx = -1; dx <= 1; ++dx)
        {
            ...
            const float atrous_geo_factor = rtgi_settings.post_blur_geometric_luma_guiding_factor * variance_guide_ramp;
            const float temporal_geo_weight = rtgi_settings.post_blur_geometric_luma_guiding ? exp(-square(atrous_geo_factor * 2.0f * atrous_geo_log_ratio)) : 1.0f;
            ...
            const float geometric_factor = is_center ? 1.0f : effective_raylength;
            const float gauss_weight = gauss_1d_x * gauss_1d_y * geometric_factor;

            const float raw_one_sided = min(1.0f, max_allowed_y / (sample_sh_y.w + 1e-4f));
            const float one_sided_luminance_weight = rtgi_settings.post_blur_variance_guiding ?
                lerp(1.0f, raw_one_sided, variance_guide_ramp) : 1.0f;
