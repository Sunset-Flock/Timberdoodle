...
MAKE_COMPUTE_COMPILE_INFO(raymarch_clouds_volumetric_shadowmap_compile_info, "./src/rendering/volumetric/raymarch_volumetric_shadow_map.hlsl", "entry_raymarch")
...
inline void raymarch_volumetric_shadow_map_callback(daxa::TaskInterface ti, RenderContext * render_context)
{
    ...
    auto const & pipeline = render_context->gpu_context->compute_pipelines.at(raymarch_clouds_volumetric_shadowmap_compile_info().name);
    ti.recorder.set_pipeline(*pipeline);
    ti.recorder.push_constant(push);

    auto const cells = volume_size.x * volume_size.y * volume_size.z;
    auto const dispatch_size = round_up_div(cells, RAYMARCH_VOLUMETRIC_SHADOW_MAP_DISPATCH_X);
    DBG_ASSERT_TRUE_M(dispatch_size < 65536, "Max allowed dispatch size exceeded");
    ti.recorder.dispatch({dispatch_size, 1, 1});
}
