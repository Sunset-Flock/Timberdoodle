...
)
{
    ...
    let layer_freq_offset = sin((2 * PI * randomized_normalized_offset * push.aurora_globals.frequency) + (push.aurora_globals.phase_shift_per_layer * layer_idx));
    ...
    let position = float3(push.aurora_globals.start + randomized_normalized_offset * aurora_vec, push.aurora_globals.height);
    ...
    if(svdtid.x < push.aurora_globals.beam_count)
    {
        ...
        for(int segment = 1; segment < push.aurora_globals.beam_path_segment_count; segment++)
        {
            ...
            let alpha_angular_offset = get_random_zero_one() * push.aurora_globals.angle_offset_per_collision;
            // let alpha_angular_offset = 0.0;
            let beta_angular_offset = get_random_zero_one() * 2 * PI;

            let offset_length = length(step_offset);

            let B_normal = push.aurora_globals.B;
            let B_tangent = y_offset;
            ...
            // let beta_offset = float2(1.0, 0.0);
