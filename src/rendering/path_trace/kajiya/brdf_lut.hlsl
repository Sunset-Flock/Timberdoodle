...
struct SpecularBrdfEnergyPreservation {
    ...
    static float3 sample_fg_lut(float ndotv, float roughness, Texture2D<float4> lut, SamplerState sampler) {
        float2 uv = float2(ndotv, roughness) * BRDF_FG_LUT_UV_SCALE + BRDF_FG_LUT_UV_BIAS;
        return lut.SampleLevel(sampler, uv, 0).xyz;
        // return float3(ndotv, roughness, 0);
    }

    static SpecularBrdfEnergyPreservation from_brdf_ndotv(SpecularBrdf brdf, float ndotv, Texture2D<float4> lut, SamplerState sampler) {
        const float roughness = brdf.roughness;
        const float3 specular_albedo = brdf.albedo;

        float3 fg = sample_fg_lut(ndotv, roughness, lut, sampler);
